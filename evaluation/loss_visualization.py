...
for rho, group in df.groupby('rho_min'):
    plt.plot(group['n_clusters'], group['loss_percentage']*100, marker='o', label=f"rho_min = {rho}")
...
for n_clusters, group in df.groupby('n_clusters'):
    plt.plot(group['rho_min'], group['loss_percentage']*100, marker='', label=f"n_clusters = {n_clusters}")

...
for n_clusters, group in df.groupby('n_clusters'):
    plt.plot(
        ...
        group['misses_over_total_edges'] * 100,
        marker='o', linestyle='-',
        label=f"n_clusters = {n_clusters} (Total Edges)"
    )
    plt.plot(
        group['rho_min'],
        group['misses_over_total_high_corr'] * 100,
        marker='x', linestyle='--',
        label=f"n_clusters = {n_clusters} (High Corr)"
    )

plt.title("Loss Percentage vs rho_min for Different Number of Clusters",
          fontsize=14, pad=15)
plt.xlabel("Rho_min", fontsize=12)
plt.ylabel("Loss Percentage (%)", fontsize=12)
plt.legend(title="n_clusters", fontsize=10,
...
for n_clusters, group in df.groupby('n_clusters'):
    ...
    group['misses_over_total_edges'] = group['misses_over_total_edges'].shift(
        1, fill_value=0) - group['misses_over_total_edges']
    group['misses_over_total_high_corr'] = group[
        'misses_over_total_high_corr'].shift(
        1, fill_value=0) - group['misses_over_total_high_corr']
    plt.hist(group['misses_over_total_edges'], bins=10, alpha=0.5,
             label=f"n_clusters = {n_clusters} (Total Edges)")
    plt.hist(group['misses_over_total_high_corr'], bins=10,
             alpha=0.5, label=f"n_clusters = {n_clusters} (High Corr)")

plt.title("Density of Loss Percentage for Different Number of Clusters",
          fontsize=14, pad=15)

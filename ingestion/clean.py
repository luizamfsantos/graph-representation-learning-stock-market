...
2. Cleans the data by:
    ...
    - Dropping rows with missing values in 'Date', 'Symbol', and 'Adj Close'
        columns.
    - Filtering out rows where 'Adj Close' and 'Volume' are less than or equal
        to 0.
    - Renaming columns for consistency.
    - Filtering data to include only records from the year 2023 onwards.
    - Removing duplicates in a date for the same ticker by grouping data by
        'date' and 'ticker' and calculating the mean.
    - Pivoting the DataFrame to have 'date' as the index and 'ticker' as
        columns.
    - Dropping columns with more than 20% missing values.
    - Forward-filling and backward-filling missing values.
    - Calculating the percentage change of the data.
    - Standardizing the data by subtracting the mean and dividing by the
        standard deviation.
